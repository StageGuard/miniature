...
impl<T, const ALIGN: usize> AlignedBox<T, ALIGN> {
    #[inline(always)]
    pub fn try_zeroed() -> Result<Self, OutOfMemory>
        where
            T: ValidForZero,
    {
        Ok(unsafe {
...
impl<T, const ALIGN: usize> AlignedBox<[T], ALIGN> {
    #[inline]
    pub fn try_zeroed_slice(len: usize) -> Result<Self, OutOfMemory>
        where
            T: ValidForZero,
    {
        Ok(unsafe {
            let ptr = HEAP_ALLOC.alloc_zeroed(layout_upgrade_align(
...
impl<T: ?Sized, const ALIGN: usize> Drop for AlignedBox<T, ALIGN> {
    fn drop(&mut self) {
        unsafe {
            ...
            HEAP_ALLOC.dealloc(self.inner.as_ptr().cast(), layout);
        }
    }
}
